...
final class Egentify_WooCommerce_Content_Search {
    ...
    private function collect_candidates($query, array $query_profile, array $types, $limit) {
        ...
        if ('' !== $normalized_query && $normalized_query !== mb_strtolower(trim($query))) {
            ...
            foreach (array('exact' => 56, 'prefix' => 40, 'contains' => 24) as $mode => $seed) {
                foreach ($normalized_matches[$mode] as $post_id) {
                    $this->add_candidate($candidates, $post_id, 'title_' . $mode . '_seed', $seed, true);
                }
            }
        }
    ...
    private function add_candidate(array &$candidates, $post_id, $reason, $seed_score, $once = false) {
        $post_id = (int) $post_id;
        if ($post_id < 1) {
            return;
        }
        ...
        if ($once && in_array($reason, $candidates[$post_id]['matchedOn'], true)) {
            return;
        }
    ...
    private function extract_raw_tokens($value) {
        preg_match_all('/[\p{L}\p{M}\p{N}]+(?:[-\/_\.][\p{L}\p{M}\p{N}]+)*/u', (string) $value, $matches);
