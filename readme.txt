...
**While connected:** the plugin sends your widget settings when you save them, plus a weekly health check reporting your plugin, WordPress, WooCommerce, and PHP versions. When a customer completes an order containing products added through the chat, the plugin reports the order id, number, status, total, currency, and item count (no customer details) so your Egentify dashboard can show revenue from the chat.
...
Just the connection details: your Egentify project ID, the security keys used to connect, and your widget settings. Orders placed with help from the chat also carry a hidden chat reference in their order metadata. Chat conversations are not stored in WordPress. They live in your Egentify account.
